...
В самом первом рецепте мы создали туннель, в котором трафик данных не был зашифрован. Чтобы создать полностью простой текстовый туннель, мы также отключаем аутентификацию HMAC. Это может быть полезно при отладке плохого соединения, поскольку теперь можно легко отслеживать весь трафик через туннель. В этом рецепте мы рассмотрим, как это сделать. Этот тип туннеля также полезен при измерении производительности, так как это наименее ресурсоемкий туннель, который можно установить.

### Подготовка

Установите OpenVPN 2.3.9 или выше на двух компьютерах. Убедитесь, что компьютеры подключены по сети. Для этого рецепта серверный компьютер работал под управлением CentOS 6 Linux и OpenVPN 2.3.9, а клиент работал под управлением Fedora 22 Linux и OpenVPN 2.3.10.

Поскольку мы не используем шифрование, секретные ключи не нужны.

### Как это сделать...

1. Запустите процесс OpenVPN на стороне сервера (прослушивания):
...
3. Соединение будет установлено со следующими двумя предупреждениями в выводе:
...
>   ... ******* ПРЕДУПРЕЖДЕНИЕ *******: указан нулевой шифр, шифрование не будет использоваться ...
  **** *** ПРЕДУПРЕЖДЕНИЕ *******: указан нулевой MAC, аутентификация не будет использоваться

### Как это устроено...

При такой настройке шифрование абсолютно не выполняется. Весь трафик, который отправляется через туннель, инкапсулируется в пакет OpenVPN, а затем отправляется как есть.

### Ещё больше...

Для реального просмотра трафика мы можем использовать `tcpdump`; Следуйте этим шагам:

1. Установите соединение, как описано.
2. Запустите `tcpdump` и прослушивайте сетевой интерфейс, а не сам туннельный интерфейс:
```
[root@client]# tcpdump -l -w - -i eth0 -s 0 host openvpnserver | strings
```
3. Теперь отправьте текст через туннель, используя что-то вроде `nc` (Netcat). Сначала запустите nc на стороне сервера:
```
[server]$ nc -l 31000  
```
4. На стороне клиента запустите команду `nc` в режиме клиента и введите `hello` и `goodbye`:
```
[client]$ nc 10.200.0.1 3100
hello
goodbye
```
5. В окне `tcpdump` теперь вы должны увидеть следующее:

![](pics/pic1-3.png)

6. Нажмите _Ctrl + C_ чтобы завершить `tcpdump`, а также `nc`.
