...
Если мы сможем найти согласованные ключи на стороне сервера. Обратите внимание, что вывод был переформатирован для ясности:

... Статическое шифрование: шифр BF-CBC, инициализированный 128-битным ключом ... Статическое шифрование: КЛЮЧ ШИФРА : 80797ddc 547fbdef 79eb353f 2a1f3d1f ... Статическое шифрование: использование хэш-сообщений 160-битного SHA1 для аутентификации HMAC ... Статическое шифрование: HMAC KEY: c752f254 cc4ac230 83bd8daf 6141e73d 844764d8 ... Статическое дешифрование: шифр BF-CBC ', инициализированный 128-битным ключом ... Статическое дешифрование: CIPHER KEY: 8cf9abdd 371392b1 14b5A ' H3S '255' сообщения 253 ... Статическая расшифровка: HMAC КЛЮЧ: 39e06d8e 20c0d3c6 0f63b3e7 d94f35af bd744b27 На стороне клиент мы найдем те же ключи, но ключи "шифровать" и "Расшифровать" были бы перевернуты:              

... Статическое шифрование: шифр «BF-CBC» инициализирован 128-битным ключом ... Статическое шифрование: КЛЮЧ ШИФРА : 8cf9abdd 371392b1 14b51523 25302c99 ... Статическое шифрование: Использование 160-битного хэша сообщения «SHA1» для аутентификации HMAC .. . Статическое шифрование: HMAC KEY: 39e06d8e 20c0d3c6 0f63b3e7 d94f35af bd744b27 ... Статическое дешифрование: BF-шифр CBC, инициализированная 128-битный ключ ... Статическое дешифрование: CIPHER KEY: 80797ddc 547fbdef 79eb3 шифрование, 1 битых-160 : 1 шифрование: 1 хэш сообщения 'SHA1' для аутентификации HMAC ... Статическая расшифровка: HMAC КЛЮЧ: c752f254 cc4ac230 83bd8daf 6141e73d 844764d8 Если в внимательно посмотрите на ключи, вы увидите, что каждый из них зеркально отражен на стороне клиента и сервера .              
...
$ openvpn --genkey --secret secret.key Размер файла статического ключа OpenVPN составляет 2048 бит. Каждый из ключей шифрования имеет 128 бит, тогда как ключи HMAC имеют 160 бит каждый, что в сумме составляет 776 бит. Это позволяет OpenVPN легко генерировать четыре случайных ключа из файла статического ключа, даже если для этого требуется больший ключ инициализации.              

### Ещё больше ...

Когда используется параметр tls-auth ta.key.

### Смотрите также

Настройка ключей открытых и закрытых ключей из главы 2, клиент-сервер, только в которой ключ-TLS аутентификация генерируется очень похожим образом     
